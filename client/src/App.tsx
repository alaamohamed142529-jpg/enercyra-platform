...
import { calculateEstimate } from "../../shared/enercyra";
...
function Home({ text, lang }: { text: Copy; lang: Lang }) {
  return <div className="page home-page">
    <section className="hero container">
      ...
      <div className="hero-visual"><WasteOrbit /><div className="orbit-label classify-label"><Sparkles size={14} />{lang === "ar" ? "صنّف" : "CLASSIFY"}</div><div className="orbit-label calculate-label"><BarChart3 size={14} />{lang === "ar" ? "احسب" : "CALCULATE"}</div><div className="orbit-label connect-label"><UsersRound size={14} />{lang === "ar" ? "تواصل" : "CONNECT"}</div></div>
    </section>
...
function Classify({ text, lang }: { text: Copy; lang: Lang }) {
  ...
  const onFile = (name: string) => { setFileName(name); window.sessionStorage.setItem("enercyra-classification", JSON.stringify({ classId: "plastic", displayNameEn: "Plastic", displayNameAr: "بلاستيك", imageName: name })); toast.success(lang === "ar" ? "تم اختيار الصورة — جاهز للتحليل" : "Image selected — ready to analyze"); };
  return <div className="page container narrow-page"><div className="page-heading"><span className="eyebrow"><Sparkles size={15} />{lang === "ar" ? "الخطوة الأولى" : "Step one"}</span><h1>{lang === "ar" ? "حلّل نفاياتك بالذكاء الاصطناعي." : "Classify your waste."}</h1><p>{lang === "ar" ? "ارفع صورة وسيحدد نظام إنِرسيرا نوع المادة." : "Upload an image and let Enercyra identify the material."}</p></div><div className="classify-grid"><label className="upload-card"><input type="file" accept="image/*" capture="environment" onChange={(event) => event.target.files?.[0] && onFile(event.target.files[0].name)} /><CloudUpload size={46} /><strong>{fileName || text.uploadTitle}</strong><span>{fileName ? fileName : text.uploadSub}</span><button type="button" className="btn btn-primary" onClick={() => document.querySelector<HTMLInputElement>('input[type="file"]')?.click()}><Upload size={17} />{text.choose}</button><button type="button" className="btn btn-secondary" onClick={() => document.querySelector<HTMLInputElement>('input[type="file"]')?.click()}><Camera size={17} />{text.camera}</button><small><ShieldCheck size={14} />{lang === "ar" ? "بياناتك لا تُعرض دون إذنك." : "Your data is not published without your permission."}</small></label><div className="benefit-panel"><h3>{lang === "ar" ? "ما الذي ستحصل عليه؟" : "What you’ll get"}</h3><Benefit icon={<Leaf />} title={lang === "ar" ? "فئة النفايات" : "Waste category"} copy={lang === "ar" ? "اسم عربي وإنجليزي واضح." : "Clear Arabic and English names."} /><Benefit icon={<Sparkles />} title={lang === "ar" ? "درجة الثقة" : "Confidence score"} copy={lang === "ar" ? "مؤشر بصري سهل الفهم." : "A visual confidence ring."} /><Benefit icon={<BarChart3 />} title={lang === "ar" ? "القيمة والطاقة" : "Value & energy"} copy={lang === "ar" ? "تقديرات مرجعية مع تنبيه واضح." : "Reference estimates with a disclaimer."} /><div className="progress-steps"><span className="active">1</span><i /><span>2</span><i /><span>3</span><p>{lang === "ar" ? "ارفع، حلّل، افهم" : "Upload, analyze, understand"}</p></div></div></div><div className="demo-note"><Lightbulb size={17} /><span>{lang === "ar" ? "في النسخة التجريبية، يمكنك فتح نتيجة مثال قبل ربط endpoint الخاص بالموديل." : "In the visual MVP, you can open an example result before the Python inference endpoint is connected."}</span><button className="text-button" onClick={() => navigate("/result")}>{lang === "ar" ? "عرض المثال" : "View example"}<ArrowRight size={15} /></button></div></div>;
}
...
function Result({ text, lang }: { text: Copy; lang: Lang }) {
  ...
  const [, navigate] = useLocation();
  const { user } = useAuth();
  const item = referenceData.find((entry) => entry.id === "plastic") ?? referenceData[0];
  const estimate = calculateEstimate(Number(weight || 0), { priceEgpPerKg: item.price, lhvMjPerKg: item.lhv, status: item.status as "reference" | "pending" });
  const energy = estimate.energyMj;
  return <div className="page container result-page"><div className="page-heading compact"><span className="eyebrow"><Check size={15} />{lang === "ar" ? "تحليل مكتمل" : "Analysis complete"}</span><h1>{lang === "ar" ? "نتيجة تصنيف الذكاء الاصطناعي" : "AI Classification Result"}</h1><p>{lang === "ar" ? "توضح هذه الشاشة نتيجة التصنيف والتقديرات المرجعية." : "This screen shows the classification result and reference estimates."}</p></div><div className="result-card"><div className="material-preview"><div className="bottle-illustration">PET</div><small>{lang === "ar" ? "الصورة المرفوعة" : "Uploaded image"}</small></div><div className="result-main"><h2>{lang === "ar" ? item.ar : item.en}</h2><p>{lang === "ar" ? "الفئة المتوقعة" : "Predicted category"}: <strong>{lang === "ar" ? item.ar : item.en}</strong></p><div className="confidence-ring"><div><strong>96.8%</strong><small>{lang === "ar" ? "ثقة" : "confidence"}</small></div></div><span className="confidence-pill"><ShieldCheck size={15} />{lang === "ar" ? "ثقة عالية" : "High confidence"}</span></div><div className="material-details"><h3>{lang === "ar" ? "تفاصيل المادة" : "Material Details"}</h3><Detail icon={<Leaf />} label={lang === "ar" ? "الفئة" : "Category"} value={lang === "ar" ? "قابلة لإعادة التدوير" : item.category} /><Detail icon={<Tag />} label={lang === "ar" ? "المادة" : "Material"} value="PET" /><Detail icon={<Zap />} label={lang === "ar" ? "الاحتراق" : "Combustible"} value={lang === "ar" ? "مرجعي" : "Reference"} /><div className="notice"><CircleHelp size={16} />{lang === "ar" ? "كل القيم أدناه تقديرات مرجعية وقد تختلف حسب السوق وجودة المادة." : "All figures below are Reference Estimates and may vary by market and material quality."}</div></div></div><div className="calculation-panel"><div className="calc-intro"><div className="feature-icon"><BarChart3 /></div><div><h3>{lang === "ar" ? "احسب القيمة والطاقة" : "Calculate Your Value"}</h3><p>{lang === "ar" ? "أدخل الوزن لمعرفة التقدير المرجعي." : "Enter the weight to see a reference estimate."}</p></div></div><label>{lang === "ar" ? "الوزن (كجم)" : "Weight (kg)"}<input type="number" min="0" value={weight} onChange={(event) => setWeight(event.target.value)} /></label><Metric lang={lang} label={lang === "ar" ? "السعر المرجعي" : "Reference price"} value="—" unit="EGP/kg" pending /><Metric lang={lang} label={lang === "ar" ? "القيمة التقديرية" : "Estimated value"} value="—" unit="EGP" pending /><Metric lang={lang} label={lang === "ar" ? "الطاقة التقديرية" : "Estimated energy"} value={estimate.energyKwh !== null ? estimate.energyKwh.toFixed(1) : "—"} unit="kWh" pending={!energy} /></div><div className="result-actions"><button className="btn btn-primary" onClick={() => { if (user) navigate("/publish"); else { toast.info(lang === "ar" ? "يجب تسجيل الدخول للنشر." : "Sign in is required to publish."); startLogin(); } }}><PackageSearch size={17} />{lang === "ar" ? "انشر في السوق" : "Publish to Marketplace"}<ArrowRight size={17} /></button><button className="btn btn-secondary" onClick={() => navigate("/classify")}><Upload size={17} />{lang === "ar" ? "صنّف صورة أخرى" : "Classify Another Image"}</button></div></div>;
}
...
function Metric({ lang, label, value, unit, pending }: { lang: Lang; label: string; value: string; unit: string; pending?: boolean }) { return <div className="metric"><small>{label}<i>ⓘ</i></small><strong>{value} <em>{unit}</em></strong><span className={pending ? "pending" : "reference-tag"}>{pending ? (lang === "ar" ? "تقدير مرجعي" : "Reference Estimate") : (lang === "ar" ? "محسوب" : "Calculated")}</span></div>; }
...
function Publish({ lang }: { text: Copy; lang: Lang }) {
  ...
  const [classification] = useState<{ classId: string; displayNameEn: string; displayNameAr: string; imageName?: string }>(() => { try { return JSON.parse(window.sessionStorage.getItem("enercyra-classification") || "null") || { classId: "plastic", displayNameEn: "Plastic", displayNameAr: "بلاستيك" }; } catch { return { classId: "plastic", displayNameEn: "Plastic", displayNameAr: "بلاستيك" }; } });
  ...
  return <div className="page container narrow-page"><div className="page-heading"><span className="eyebrow"><Tag size={15} />{lang === "ar" ? "معاينة النشر" : "Publish preview"}</span><h1>{lang === "ar" ? "انشر مادة في السوق." : "Publish material."}</h1><p>{lang === "ar" ? "هذه معاينة لمسار النشر قبل توصيل نموذج التصنيف الحقيقي. أضف التفاصيل التي تساعد المشتري المحتمل على فهم الإعلان." : "This is a preview of the publish flow before the real classifier is connected. Add the details a potential buyer needs to understand the listing."}</p></div><form className="benefit-panel publish-form" onSubmit={(event) => { event.preventDefault(); createListing.mutate({ classId: classification.classId, displayNameEn: classification.displayNameEn, displayNameAr: classification.displayNameAr, weightKg: Number(weight), location, condition: condition || undefined, notes: notes || undefined, imageMetadata: classification.imageName ? JSON.stringify({ fileName: classification.imageName, capturedAt: new Date().toISOString() }) : undefined }); }}><div className="form-grid"><label>{lang === "ar" ? "المادة" : "Material"}<input value={lang === "ar" ? classification.displayNameAr : classification.displayNameEn} readOnly /></label><label>{lang === "ar" ? "الوزن (كجم)" : "Weight (kg)"}<input type="number" min="0.1" step="0.1" value={weight} onChange={(event) => setWeight(event.target.value)} required /></label><label>{lang === "ar" ? "الموقع" : "Location"}<input placeholder={lang === "ar" ? "مثال: القاهرة" : "e.g. Cairo"} value={location} onChange={(event) => setLocation(event.target.value)} required /></label><label>{lang === "ar" ? "الحالة" : "Condition"}<input placeholder={lang === "ar" ? "نظيفة / مختلطة" : "Clean / mixed"} value={condition} onChange={(event) => setCondition(event.target.value)} /></label></div><label>{lang === "ar" ? "ملاحظات" : "Notes"}<textarea rows={5} placeholder={lang === "ar" ? "أضف أي تفاصيل عن المادة" : "Add details about the material"} value={notes} onChange={(event) => setNotes(event.target.value)} /></label><div className="notice"><CircleHelp size={16} />{lang === "ar" ? "سيظهر الإعلان كقيمة مرجعية أو سعر قيد التجهيز، ولا يمثل عرضًا تجاريًا ملزمًا." : "The listing will show a reference or pending price and is not a binding commercial offer."}</div><div className="result-actions"><button className="btn btn-primary" type="submit" disabled={createListing.isPending}><PackageSearch size={17} />{createListing.isPending ? (lang === "ar" ? "جارٍ النشر" : "Publishing") : (lang === "ar" ? "نشر الإعلان" : "Publish listing")}</button><button className="btn btn-secondary" type="button" onClick={() => navigate("/result")}><ArrowLeft size={17} />{lang === "ar" ? "رجوع" : "Back"}</button></div></form></div>;
}

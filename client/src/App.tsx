...
function ListingCard({ item, lang, text }: { item: MarketplaceListing; lang: Lang; text: Copy }) { return <article className="listing-card"><img className="listing-image" src={item.imageUrl || ""} alt={lang === "ar" ? item.displayNameAr : item.displayNameEn} /><div className="listing-content"><div className="listing-title"><div><h3>{lang === "ar" ? item.displayNameAr : item.displayNameEn}</h3><small>{item.status === "Price pending" ? text.pending : text.reference}</small></div><Tag size={17} /></div><div className="listing-meta"><span><PackageSearch size={15} />{item.weightKg} kg</span><span><Globe2 size={15} />{item.location}</span></div><div className="listing-bottom"><strong>{item.status === "Price pending" ? (lang === "ar" ? "السعر قيد التجهيز" : "Price pending") : (lang === "ar" ? "قيمة مرجعية" : "Reference value")}</strong><button onClick={() => toast.info(item.contactPhone || (lang === "ar" ? "لا يوجد رقم تواصل محفوظ لهذا الإعلان." : "No contact phone saved for this listing."))}><UsersRound size={15} />{lang === "ar" ? "تواصل" : "Contact"}</button></div></div></article>; }

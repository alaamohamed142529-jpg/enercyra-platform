  ...
  return (
    <ThemeProvider defaultTheme="dark" switchable>
      <div className={`app-shell ${dark ? "theme-dark" : "theme-light"}`} dir={dir}>
        <SiteHeader lang={lang} setLang={setLang} dark={dark} setDark={setDark} currency={currency} setCurrency={setCurrency} text={text} />
        <main>
          <Switch>
            <Route path="/" component={() => <Home text={text} lang={lang} />} />
            <Route path="/classify" component={() => <Classify text={text} lang={lang} />} />
            <Route path="/result" component={() => <Result text={text} lang={lang} currency={currency} />} />
...
function SiteHeader({ lang, setLang, dark, setDark, currency, setCurrency, text }: { lang: Lang; setLang: (value: Lang) => void; dark: boolean; setDark: (value: boolean) => void; currency: Currency; setCurrency: (value: Currency) => void; text: Copy }) {
  ...
  return (
    <header className="site-header">
      ...
      <div className="header-actions">
        <button className="icon-button" aria-label="Switch language" onClick={() => setLang(lang === "en" ? "ar" : "en")}><Globe2 size={17} /><span>{lang === "en" ? "العربية" : "English"}</span></button><label className="currency-control" title={lang === "ar" ? "عرض تقريبي فقط؛ المصدر الأساسي بالجنيه المصري" : "Display only; source reference remains EGP"}><span>{lang === "ar" ? "عملة العرض" : "Display"}</span><select aria-label={lang === "ar" ? "عملة العرض" : "Display currency"} value={currency} onChange={(event) => setCurrency(event.target.value as Currency)}>{(["EGP", "SAR", "USD", "EUR"] as Currency[]).map((code) => <option key={code} value={code}>{code}</option>)}</select></label>
...
function InteractiveHeroVisual({ lang }: { lang: Lang }) {
  ...
  return <div className="hero-visual hero-composed" data-hero-version="visible-connect-live-v1" aria-label={lang === "ar" ? "زجاجة بلاستيك وعلبة صفيح وزجاجة زجاج وكرتون ومخلفات عضوية حول مركز الطاقة" : "Plastic bottle, tin can, glass bottle, cardboard, and organic waste around an energy center"}>
    ...
    <a className="hero-function-label label-connect" data-hovered={connectHovered} href="/businesses" aria-label={lang === "ar" ? "تواصل مع المشترين" : "Connect with buyers"} onMouseEnter={() => setConnectHovered(true)} onMouseLeave={() => setConnectHovered(false)} onFocus={() => setConnectHovered(true)} onBlur={() => setConnectHovered(false)}><span className="connect-callout-content"><UsersRound size={13} /><span>{lang === "ar" ? "تواصل" : "CONNECT"}</span><small>{lang === "ar" ? "السوق والشركات" : "MARKETPLACE"}</small></span></a>
...
function Result({ text, lang, currency }: { text: Copy; lang: Lang; currency: Currency }) {

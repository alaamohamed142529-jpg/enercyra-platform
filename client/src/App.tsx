...
import { calculateEstimate, formatWeightKg, parseWeightInput } from "../../shared/enercyra";
import { mobileNetReferenceCatalog } from "../../shared/model-classes";
import { buildListingPayload, type EstimateSnapshot } from "../../shared/listing-payload";
...
type PublishDraft = { weight?: string; displayNameEn?: string; displayNameAr?: string; category?: string; imageDataUrl?: string; condition?: string; estimateSnapshot?: EstimateSnapshot };
...
type MarketplaceListing = { id: number; classId: string; displayNameEn: string; displayNameAr: string; weightKg: string | number; location: string; contactPhone?: string | null; condition?: string | null; notes?: string | null; imageUrl?: string | null; imageMetadata?: string | null; status: string };
...
function ListingCard({ item, lang, text }: { item: MarketplaceListing; lang: Lang; text: Copy }) {
  ...
  return <article className="listing-card listing-card-clickable" onClick={openDetails} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); openDetails(); } }} role="link" tabIndex={0} aria-label={lang === "ar" ? `عرض تفاصيل ${title}` : `View details for ${title}`}><img className="listing-image" src={item.imageUrl || ""} alt={title} /><div className="listing-content"><div className="listing-title"><div><h3>{title}</h3><small>{item.status === "Price pending" ? text.pending : text.reference}</small></div><Tag size={17} /></div><div className="listing-meta"><span><PackageSearch size={15} />{formatWeightKg(item.weightKg)} kg</span><span><Globe2 size={15} />{item.location}</span></div>{item.condition && <p className="listing-notes">{item.condition}</p>}<div className="listing-bottom"><strong>{item.status === "Price pending" ? (lang === "ar" ? "السعر قيد التجهيز" : "Price pending") : (lang === "ar" ? "قيمة مرجعية" : "Reference value")}</strong><button onClick={(event) => { event.stopPropagation(); toast.info(item.contactPhone || (lang === "ar" ? "لا يوجد رقم تواصل محفوظ لهذا الإعلان." : "No contact phone saved for this listing.")); }}><UsersRound size={15} />{lang === "ar" ? "تواصل" : "Contact"}</button></div></div></article>;
}
function ListingDetail({ lang, text }: { text: Copy; lang: Lang }) {
  ...
  const estimate = getListingEstimate(item);
  const pending = lang === "ar" ? "قيد التحقق" : "Pending verification";
  const lhv = estimate?.snapshot.originalEnergyMjPerKg ?? null;
  const price = estimate?.snapshot.referencePricePerKg ?? null;
  const energyMj = estimate?.snapshot.estimatedEnergyMj ?? null;
  const energyKwh = estimate?.snapshot.estimatedEnergyKwh ?? null;
  const valueEgp = estimate?.snapshot.estimatedValue ?? null;
  const currency = estimate?.snapshot.currency || "EGP";
  const originalEnergy = lhv === null ? (lang === "ar" ? "غير منطبق" : "N/A") : `${lhv.toFixed(1)} MJ/kg`;
  const totalEnergy = energyMj === null ? (estimate?.reference?.combustible ? pending : (lang === "ar" ? "غير منطبق" : "N/A")) : `${energyMj.toFixed(1)} MJ (${energyKwh?.toFixed(1)} kWh)`;
  const referencePrice = price === null ? pending : `${price.toFixed(2)} ${currency}/kg`;
  const estimatedValue = valueEgp === null ? pending : `${valueEgp.toFixed(2)} ${currency}`;
  return <div className="page container listing-detail-page"><button className="text-button listing-back" onClick={() => navigate("/marketplace")}><ArrowLeft size={17} />{lang === "ar" ? "العودة إلى السوق" : "Back to Marketplace"}</button><div className="listing-detail-card"><img className="listing-detail-image" src={item.imageUrl || ""} alt={title} /><div className="listing-detail-content"><span className="eyebrow"><Tag size={15} />{lang === "ar" ? "تفاصيل الإعلان" : "Listing details"}</span><h1>{title}</h1><p className="listing-detail-status">{status}</p><div className="listing-detail-grid"><div><span>{lang === "ar" ? "الوزن" : "Weight"}</span><strong>{formatWeightKg(item.weightKg)} kg</strong></div><div><span>{lang === "ar" ? "الموقع" : "Location"}</span><strong>{item.location}</strong></div><div><span>{lang === "ar" ? "السعر المرجعي" : "Reference price"}</span><strong>{referencePrice}</strong></div><div><span>{lang === "ar" ? "القيمة المقدرة" : "Estimated value"}</span><strong>{estimatedValue}</strong></div><div><span>{lang === "ar" ? "الطاقة الأصلية" : "Original energy"}</span><strong>{originalEnergy}</strong></div><div><span>{lang === "ar" ? "إجمالي الطاقة" : "Total energy"}</span><strong>{totalEnergy}</strong></div>{item.condition && <div><span>{lang === "ar" ? "الحالة" : "Condition"}</span><strong>{item.condition}</strong></div>}</div>{item.notes && <div className="listing-detail-notes"><span>{lang === "ar" ? "ملاحظات الناشر" : "Publisher notes"}</span><p>{item.notes}</p></div>}<button className="btn btn-primary" onClick={() => toast.info(item.contactPhone || (lang === "ar" ? "لا يوجد رقم تواصل محفوظ لهذا الإعلان." : "No contact phone saved for this listing."))}><UsersRound size={17} />{lang === "ar" ? "تواصل مع البائع" : "Contact seller"}</button></div></div></div>;
}
function Businesses({ lang }: { lang: Lang }) {
  ...
  return <div className="page container content-page businesses-page">
    <div className="page-heading businesses-heading"><span className="eyebrow"><Building2 size={15} />{isAr ? "دليل الأعمال البيئية" : "Verified business directory"}</span><h1>{isAr ? "الشركات" : "Businesses"}</h1><p>{isAr ? "شركات مصرية تعمل في جمع المخلفات أو إعادة تدويرها أو تحويلها إلى موارد. البيانات مأخوذة من المواقع الرسمية للشركات." : "Egyptian companies involved in collecting, recycling, recovering, or managing waste. Details are taken from official company websites."}</p><div className="business-stats"><span><strong>{egyptianBusinesses.length}</strong>{isAr ? " جهات مدرجة" : " verified listings"}</span><span><strong>{cities.length}</strong>{isAr ? " مناطق" : " areas"}</span><span><ShieldCheck size={15} />{isAr ? "بيانات منشورة علنًا" : "Publicly published data"}</span></div><div className="business-heading-actions"><a className="btn btn-secondary" href="#nearby-facilities"><MapPin size={17} />{isAr ? "اعثر على أقرب جهة مناسبة" : "Find a nearby compatible facility"}<ArrowRight size={17} /></a></div></div>
    <NearbyFacilities lang={lang} businesses={egyptianBusinesses} material={requestedMaterial} />
...
function EnergyForecast({ lang }: { lang: Lang }) {
  ...
  const labels = lang === "ar" ? { eyebrow: "تحليل إنتاج الطاقة", title: "توقّع إنتاج الطاقة من المخلفات", description: "توقّع إنتاج الطاقة القادم من المخلفات بناءً على بيانات تاريخية", placeholder: "مثال: 145, 152, 148, 160 ...", action: "توقّع إنتاج الأيام السبعة القادمة", history: "الإنتاج التاريخي", forecast: "الإنتاج المتوقع", unit: "kWh", invalidCount: "أدخل 14 قيمة يومية بالضبط.", invalidNumber: "كل القيم يجب أن تكون أرقامًا غير سالبة.", empty: "أدخل قيم إنتاج الطاقة اليومية أولًا." } : { eyebrow: "Waste-to-energy intelligence", title: "Waste Energy Output Forecast", description: "Forecast upcoming waste-derived energy output based on historical data", placeholder: "Example: 145, 152, 148, 160 ...", action: "Forecast the next 7 days of output", history: "Historical output", forecast: "Forecast output", unit: "kWh", invalidCount: "Enter exactly 14 daily values.", invalidNumber: "Every value must be a finite non-negative number.", empty: "Enter daily energy-output values first." };

...
function Metric({ icon, lang, label, value, unit, pending, empty, notApplicable, note }: { icon?: React.ReactNode; lang: Lang; label: string; value: string; unit: string; pending?: boolean; empty?: boolean; notApplicable?: boolean; note?: string }) { const shownValue = notApplicable ? (lang === "ar" ? "غير منطبق" : "N/A") : empty ? "—" : pending ? pendingMetricLabel(lang) : value; const status = notApplicable ? (lang === "ar" ? "لا توجد طاقة حرارية للمادة" : "Not an energy-recovery material") : empty ? (lang === "ar" ? "أدخل الوزن" : "Enter weight") : pending ? pendingMetricStatus(lang) : (lang === "ar" ? "محسوب" : "Calculated"); return <div className="metric"><small className="metric-label">{icon}{label}</small><strong>{shownValue} {!empty && !pending && !notApplicable && <em>{unit}</em>}</strong>{note && <span className="metric-note">{note}</span>}<span className={empty || pending || notApplicable ? "pending" : "reference-tag"}>{status}</span></div>; }

...
function ListingDetail({ lang, text }: { text: Copy; lang: Lang }) {
  ...
  const lhv = estimate?.reference.lhv ?? null;
  const price = estimate?.reference.price ?? null;
  const energyMj = estimate?.estimate.energyMj ?? null;
  const energyKwh = estimate?.estimate.energyKwh ?? null;
  const valueEgp = estimate?.estimate.valueEgp ?? null;
  ...
  const referencePrice = price === null ? pending : `${price.toFixed(2)} EGP/kg`;
  const estimatedValue = valueEgp === null ? pending : `${valueEgp.toFixed(2)} EGP`;

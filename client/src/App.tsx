...
function InteractiveHeroVisual({ lang }: { lang: Lang }) {
  return <div className="hero-visual hero-composed" data-hero-version="visible-connect-live-v1" aria-label={lang === "ar" ? "زجاجة بلاستيك وعلبة صفيح وزجاجة زجاج وكرتون ومخلفات عضوية حول مركز الطاقة" : "Plastic bottle, tin can, glass bottle, cardboard, and organic waste around an energy center"}>
    ...
    <div className="interactive-material-orbit" role="img" aria-label={lang === "ar" ? "زجاجة بلاستيك وعلبة صفيح وزجاجة زجاج وكرتون ومخلفات عضوية تتحرك حول مركز طاقة" : "Plastic bottle, tin can, glass bottle, cardboard, and organic waste moving around an energy core"}>
      ...
      <span className="material-object material-bottle"><img src="/manus-storage/enercyra-plastic-bottle-hero_1a2f45aa.webp" alt="" aria-hidden="true" /></span><span className="material-object material-can"><img src="/manus-storage/enercyra-can-hero_941405f5.webp" alt="" aria-hidden="true" /></span><span className="material-object material-glass"><img src="/manus-storage/enercyra-glass-bottle-clear-v2_0adf2f18.webp" alt="" aria-hidden="true" /></span><span className="material-object material-cardboard"><img src="/manus-storage/enercyra-cardboard-hero_14d3326e.webp" alt="" aria-hidden="true" /></span><span className="material-object material-organic"><img src="/manus-storage/enercyra-organic-hero_5ad406e3.webp" alt="" aria-hidden="true" /></span>
      <span className="energy-core"><BrandMark /><span className="core-pulse" /></span>
    </div>
    <div className="hero-function-label label-classify"><Sparkles size={13} /><span>{lang === "ar" ? "تصنيف" : "CLASSIFY"}</span></div><div className="hero-function-label label-calculate"><BarChart3 size={13} /><span>{lang === "ar" ? "حساب" : "CALCULATE"}</span></div>
    <div className="hero-corner corner-tl" aria-hidden="true" /><div className="hero-corner corner-br" aria-hidden="true" />
  </div>;
}

function FeatureCard({ href, icon, title, copy: description }: { href: string; icon: React.ReactNode; title: string; copy: string }) { return <Link href={href} className="feature-card"><div className="feature-icon">{icon}</div><h3>{title}</h3><p>{description}</p><ArrowRight className="feature-arrow" size={18} /></Link>; }
...
function RecentAnalysis({ lang }: { lang: Lang }) {
  ...
  return <div className="recent-card"><div className="recent-header"><span><BarChart3 size={17} />{lang === "ar" ? "أحدث تحليل" : "Recent Analysis"}</span><Link href={classification ? "/result" : "/classify"}>{classification ? (lang === "ar" ? "عرض" : "View") : (lang === "ar" ? "ابدأ" : "Start")}<ArrowRight size={15} /></Link></div>{classification && item ? <div className="recent-result"><div className="recent-row"><div className="recent-image">{classification.imageDataUrl ? <img src={classification.imageDataUrl} alt={name} /> : <div className="mini-material">{item.id.slice(0, 4).toUpperCase()}</div>}</div><div><strong>{name}</strong><small>{confidenceLabel} · {confidence}</small>{classification.imageName && <small className="recent-filename">{classification.imageName}</small>}</div><div className="recent-stat"><small>{lang === "ar" ? "حالة القيمة" : "Value status"}</small><strong>{item.price !== null ? (lang === "ar" ? "متاح" : "Available") : "—"}</strong></div><div className="recent-stat"><small>{lang === "ar" ? "حالة الطاقة" : "Energy status"}</small><strong>{item.lhv !== null ? (lang === "ar" ? "متاح" : "Available") : "—"}</strong></div></div><Link className="recent-connect" href="/businesses"><UsersRound size={15} />{lang === "ar" ? "تواصل" : "Connect"}<ArrowRight size={14} /></Link></div> : <div className="recent-empty"><strong>{lang === "ar" ? "لا توجد تحليلات بعد" : "No analyses yet"}</strong><small>{lang === "ar" ? "ارفع صورة لتظهر أحدث نتيجة هنا." : "Upload an image to show your latest result here."}</small></div>}</div>;
}

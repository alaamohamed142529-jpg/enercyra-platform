...
function ListingDetail({ lang, text }: { text: Copy; lang: Lang }) {
  ...
  return <div className="page container listing-detail-page"><button className="text-button listing-back" onClick={() => navigate("/marketplace")}><ArrowLeft size={17} />{lang === "ar" ? "العودة إلى السوق" : "Back to Marketplace"}</button><div className="listing-detail-card"><img className="listing-detail-image" src={item.imageUrl || ""} alt={title} /><div className="listing-detail-content"><span className="eyebrow"><Tag size={15} />{lang === "ar" ? "تفاصيل الإعلان" : "Listing details"}</span><h1>{title}</h1><p className="listing-detail-status">{status}</p><div className="listing-detail-grid"><div><span>{lang === "ar" ? "الوزن" : "Weight"}</span><strong>{formatWeightKg(item.weightKg)} kg</strong></div><div><span>{lang === "ar" ? "الموقع" : "Location"}</span><strong>{item.location}</strong></div>{item.condition && <div><span>{lang === "ar" ? "الحالة" : "Condition"}</span><strong>{item.condition}</strong></div>}</div>{item.notes && <div className="listing-detail-notes"><span>{lang === "ar" ? "ملاحظات الناشر" : "Publisher notes"}</span><p>{item.notes}</p></div>}<button className="btn btn-primary" onClick={() => toast.info(item.contactPhone || (lang === "ar" ? "لا يوجد رقم تواصل محفوظ لهذا الإعلان." : "No contact phone saved for this listing."))}><UsersRound size={17} />{lang === "ar" ? "تواصل مع البائع" : "Contact seller"}</button></div></div></div>;
}

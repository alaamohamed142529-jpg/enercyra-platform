...
function Result({ text, lang }: { text: Copy; lang: Lang }) {
  ...
  return <div className="page container result-page" data-result-version="notebook-reference-v1"><div className="page-heading compact"><span className="eyebrow"><Check size={15} />{lang === "ar" ? "تحليل مكتمل" : "Analysis complete"}</span><h1>{lang === "ar" ? "نتيجة تصنيف الذكاء الاصطناعي" : "AI Classification Result"}</h1><p>{lang === "ar" ? "توضح هذه الشاشة نتيجة MobileNetV3 والتقديرات المرجعية." : "This screen shows the MobileNetV3 result and reference estimates."}</p></div><div className="result-card"><div className="material-preview">{classification.imageDataUrl ? <img className="uploaded-preview" src={classification.imageDataUrl} alt={lang === "ar" ? "الصورة المحللة" : "Analyzed waste"} /> : <div className="bottle-illustration">{item.en.slice(0, 4).toUpperCase()}</div>}<small>{classification.imageName || (lang === "ar" ? "نتيجة تجريبية" : "Demo result")}</small></div><div className="result-main"><h2>{lang === "ar" ? item.ar : item.en}</h2><p>{lang === "ar" ? "الفئة المكتشفة" : "Detected category"}: <strong>{lang === "ar" ? item.ar : item.en}</strong></p><div className="confidence-ring"><div><strong>{(confidence * 100).toFixed(1)}%</strong><small>{lang === "ar" ? "ثقة النموذج" : "model confidence"}</small></div></div><span className="confidence-pill"><ShieldCheck size={15} />{confidenceLabel}</span></div><div className="material-details"><h3>{lang === "ar" ? "تفاصيل المادة" : "Material Details"}</h3><Detail icon={<Leaf />} label={lang === "ar" ? "الفئة" : "Category"} value={lang === "ar" ? item.category : item.category} /><Detail icon={<Tag />} label={lang === "ar" ? "المادة" : "Material"} value={lang === "ar" ? item.ar : item.en} /><Detail icon={<Zap />} label={lang === "ar" ? "إمكانية الاسترداد الطاقي" : "Energy recovery potential"} value={energyRecoveryLabel(lang, item.combustible)} /><div className="notice"><CircleHelp size={16} />{lang === "ar" ? "كل القيم أدناه تقديرات مرجعية وقد تختلف حسب السوق وجودة المادة." : "All figures below are Reference Estimates and may vary by market and material quality."}</div></div></div><div className="calculation-panel"><div className="calc-intro"><div className="feature-icon"><BarChart3 /></div><div><h3>{lang === "ar" ? "احسب القيمة والطاقة" : "Calculate Your Value"}</h3><p>{lang === "ar" ? "أدخل الوزن لمعرفة التقدير المرجعي." : "Enter the weight to see a reference estimate."}</p></div></div>{item.status === "reference" ? <div className="notice reference-data-note"><CircleHelp size={16} />{notebookReferenceNotice(lang)}</div> : <div className="notice pending-data-note"><CircleHelp size={16} />{pendingDataNotice(lang)}</div>}<label>{lang === "ar" ? "الوزن (كجم)" : "Weight (kg)"}<input type="text" inputMode="decimal" pattern="[0-9٠-٩.,٫]*" minLength={1} value={weight} placeholder={lang === "ar" ? "مثال: 2.5" : "e.g. 2.5"} autoComplete="off" aria-describedby="weight-help" onInput={(event) => setWeight(event.currentTarget.value)} onChange={(event) => setWeight(event.currentTarget.value)} /><small id="weight-help" className="field-help">{lang === "ar" ? "يمكنك إدخال أي وزن موجب، مثل 2 أو 10.5 كجم." : "Enter any positive weight, such as 2 or 10.5 kg."}</small></label><Metric lang={lang} label={lang === "ar" ? "السعر المرجعي" : "Reference price"} value={item.price !== null ? item.price.toFixed(2) : "—"} unit="EGP/kg" pending={item.price === null} /><Metric lang={lang} label={lang === "ar" ? "القيمة الحرارية LHV" : "LHV per kg"} value={item.lhv !== null ? item.lhv.toFixed(1) : "—"} unit="MJ/kg" pending={item.lhv === null} /><Metric lang={lang} label={lang === "ar" ? "القيمة التقديرية" : "Estimated value"} value={hasWeight && estimate.valueEgp !== null ? estimate.valueEgp.toFixed(2) : "—"} unit="EGP" pending={estimate.valueEgp === null} empty={!hasWeight} /><Metric lang={lang} label={lang === "ar" ? "الطاقة الكلية" : "Total energy"} value={hasWeight && estimate.energyMj !== null ? estimate.energyMj.toFixed(1) : "—"} unit="MJ" pending={estimate.energyMj === null} empty={!hasWeight} /><Metric lang={lang} label={lang === "ar" ? "الطاقة بالكيلوواط" : "Energy equivalent"} value={hasWeight && estimate.energyKwh !== null ? estimate.energyKwh.toFixed(1) : "—"} unit="kWh" pending={estimate.energyKwh === null} empty={!hasWeight} /></div><div className="result-actions"><button className="btn btn-primary" onClick={() => { if (user) navigate("/publish"); else { toast.info(lang === "ar" ? "يجب تسجيل الدخول للنشر." : "Sign in is required to publish."); startLogin(); } }}><PackageSearch size={17} />{lang === "ar" ? "انشر في السوق" : "Publish to Marketplace"}<ArrowRight size={17} /></button><button className="btn btn-secondary" onClick={() => navigate("/classify")}><Upload size={17} />{lang === "ar" ? "صنّف صورة أخرى" : "Classify Another Image"}</button></div></div>;
}
...
function Metric({ lang, label, value, unit, pending, empty }: { lang: Lang; label: string; value: string; unit: string; pending?: boolean; empty?: boolean }) { const shownValue = empty ? "—" : pending ? pendingMetricLabel(lang) : value; const status = empty ? (lang === "ar" ? "أدخل الوزن" : "Enter weight") : pending ? pendingMetricStatus(lang) : (lang === "ar" ? "محسوب" : "Calculated"); return <div className="metric"><small>{label}<i>ⓘ</i></small><strong>{shownValue} {!empty && !pending && <em>{unit}</em>}</strong><span className={empty || pending ? "pending" : "reference-tag"}>{status}</span></div>; }

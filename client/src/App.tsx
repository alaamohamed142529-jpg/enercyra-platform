...
function Publish({ lang }: { text: Copy; lang: Lang }) {
  ...
  const [weight, setWeight] = useState(() => { try { const draft = JSON.parse(window.sessionStorage.getItem("enercyra-publish-draft") || "null"); return typeof draft?.weight === "string" ? draft.weight : ""; } catch { return ""; } });
  const [condition, setCondition] = useState(() => { try { const stored = JSON.parse(window.sessionStorage.getItem("enercyra-classification") || "null"); return typeof stored?.condition === "string" ? stored.condition : ""; } catch { return ""; } });
  const [material, setMaterial] = useState(() => { try { const stored = JSON.parse(window.sessionStorage.getItem("enercyra-classification") || "null"); return lang === "ar" ? stored?.displayNameAr || "بلاستيك" : stored?.displayNameEn || "Plastic"; } catch { return lang === "ar" ? "بلاستيك" : "Plastic"; } });
